...
draw width height rockPoints =
  let emptyPoints = reverse $ getRectangle width height
...
getHighestPoint points =
  let rows = Set.map (getRow) points
...
cantMove points shape dir =
  let nudged = nudge dir shape
...
dropShape (points, dirs, shape : shapes) =
  let shapePoints = spawnShape points shape
