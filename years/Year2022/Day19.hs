...
data Blueprint = Blueprint { _blueprintId :: Int
                           , _costs :: Map.Map Material (Map.Map Material Int) } deriving (Eq, Show)

data State = State { _minute :: Int
                   , _robots :: Map.Map Material Int
                   , _resources :: Map.Map Material Int } deriving (Eq, Show)
...
inputParser = blueprintParser `sepEndBy` eol
...
maxResourcesNeeded :: Blueprint -> Map.Map Material Int
...
partB input = let
    blueprint = input !! 0
    state = startState
    in show $ fromJust $ tryToBuy blueprint state Ore
