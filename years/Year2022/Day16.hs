...
getCosts graph =
  let helpful = helpfulValves graph
...
visitDestination graph costs state destination =
  let origin = location state
...
totalPressureAtEnd state =
  let timeRemaining = 30 - minute state
   in totalPressure state + (timeRemaining * pressurePerMinute state)

visitAll :: Graph -> Costs -> State -> State
visitAll graph costs state =
  let origin = location state :: String
