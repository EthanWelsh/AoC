...
asMap m =
  let ps = pruneMatching [isWall] (allPoints m)
      isWall = testPoint m (== '#')
...
findNextIntersection m prev curr =
  let ns = (m M.! curr) \\ [prev]
      addOne (p, c) = (p, c + 1)
...
longestPath m isEnd steps p
  ...
  | otherwise =
      let ns = filter (\(pp, _) -> pp `notElem` map fst steps) (m M.! p)
