...
calculateScore input f =
  let games = sortBy (\(Game a _) (Game b _) -> f a b) input
      rankedGames = zip [1 ..] games :: [(Int, Game)]
      valuePerGame = map (\(rank, Game _ bid) -> rank * bid) rankedGames :: [Int]
   in sum valuePerGame

comparePt1 :: Hand -> Hand -> Ordering
comparePt1 a@(Hand aa) b@(Hand bb) =
  let topRank = maximum . kinds
