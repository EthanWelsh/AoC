...
getIncrementalSets c = let
    callsAsSets = map (Set.fromList . flip Data.List.take c) [0..length c]
    in zip callsAsSets (Set.empty : callsAsSets)

winningBoards :: (Set Int, Set Int) -> [Board] -> (Set Int, [Board])
winningBoards (currentCalls, prevCalls) bs = let
    isNewWin b = isWin currentCalls b && not (isWin prevCalls b)
    in (currentCalls, filter isNewWin bs)

getCallsToWinEachBoard :: [Board] -> Calls -> [(Set Int, Board)]
getCallsToWinEachBoard bs c = let
    callSets = getIncrementalSets c
...
firstToWin bs c = let
    callsToWinEachBoard = getCallsToWinEachBoard bs c
