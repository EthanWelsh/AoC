...
math m x y a f =
  let v = f (getValue m x) (getValue m y)
   in set m a v
...
runInstruction m _i@(Add a b r) =
  m
    { getMemory = math (getMemory m) a b r (+),
...
runInstruction m _i@(Input a) =
  m
    { getMemory = set (getMemory m) a (head $ getInput m),
      getIp = getIp m @+ instructionSize _i,
      getInput = tail $ getInput m
    }
runInstruction m _i@(Output a) =
  let v = getValue (getMemory m) a
   in m
        { getIp = getIp m @+ instructionSize _i,
          getOutput = v : getOutput m
        }
runInstruction m _i@(JumpIfTrue a b) =
  let testValue = getValue (getMemory m) a
...
runInstruction m _i@(JumpIfFalse a b) =
  let testValue = getValue (getMemory m) a
      ipIfJump = Address $ getValue (getMemory m) b
      ipIfNotJump = getIp m @+ instructionSize _i
   in m {getIp = if testValue == 0 then ipIfJump else ipIfNotJump}
runInstruction m _i@(LessThan a b r) =
  m
    { getMemory = math (getMemory m) a b r (\x y -> if x < y then 1 else 0),
      getIp = getIp m @+ instructionSize _i
    }
...
instructionAtAddress m a =
  let (opcode, ops) = extractOperands m a
   in case opcode of
